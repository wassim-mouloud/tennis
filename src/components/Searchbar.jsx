...
function Searchbar({setOpenSearch, openSearch, tournaments, searched, setSearched, setName, setTournamentCity,setTournamentStart, setTournamentEnd, setTournamentSurface,setTournamentCountry, setOpenTourney}) {
  ...
  return (
    <div className='relative'>
        <div onClick={()=>setOpenSearch(true)}  className={` absolute top-[20px]  z-[40]   rounded-[15px] cursor-pointer transition-all duration-300 flex-col  ${openSearch?" w-[90vw] lg:w-[50vw] h-[60px]  right-[20px]  bg-[rgba(255,255,255,0.5)] ":" w-[50px] h-[40px] right-[20px] bg-[rgba(255,255,255,0.5)]"}`} >
                ...
                {searched.length!=0 &&  searched.map((tourney, index)=>{
                  ...
                  >
                    <div className='flex h-[70px] text-[14px]  items-center  ' >
                      <img className='object-cover h-full md:w-[10%] w-[30%] rounded-[15px] p-1' src={tourney.surface.includes('Hard')?'./hard_court.jpeg':tourney.surface.includes('Clay')?'./clay_court.webp':tourney.surface.includes('Grass')?'./grass_court.jpeg':null} alt="" />
