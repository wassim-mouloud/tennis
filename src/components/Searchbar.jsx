  ...
  return (
    <div className='relative'>
      <div onClick={() => setOpenSearch(true)}  
        className={`absolute top-[20px] z-[40] rounded-[15px] cursor-pointer transition-all duration-300 flex-col  
                     ${openSearch ? "w-[90vw] lg:w-[50vw] h-[60px] right-[20px] bg-[rgba(255,255,255,0.5)]" 
                                  : "w-[50px] h-[40px] right-[20px] bg-[rgba(255,255,255,0.5)]"}`}
      >
        <div className='flex items-center justify-center h-full bg-[rgba(255,255,255,0.5)] rounded-[15px]'>
          {openSearch && 
            <input onChange={handleChange} 
                   ...
                   className='bg-[rgba(255,255,255,0.1)] order-2 w-[calc(100%-34px)] h-[100%] rounded-r-[15px] outline-none outline-0 focus:outline-none focus:outline-0 text-[20px] p-4 md:text-[24px]'/>
          }
          <img src="./search.png" 
               alt="search-icon" 
               className={`h-[24px] w-[24px] order-1 object-cover ${openSearch ? "m-[10px]" : ''}`} 
          />
        </div>
        {openSearch && (
              ...
              return (
                ...
                >
                  <div className='flex h-[70px] text-[14px] items-center'>
                    <img className='object-cover h-full md:w-[10%] w-[30%] rounded-[15px] p-1' 
                         src={tourney.surface.includes('Hard') ? './hard_court.webp'
